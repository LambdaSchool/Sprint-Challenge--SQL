...
INSERT INTO channel_users (channel_id, user_id) VALUES (1, 1); --Alice/general
INSERT INTO channel_users (channel_id, user_id) VALUES (1, 2); --Bob/general
INSERT INTO channel_users (channel_id, user_id) VALUES (1, 3); --Chris/general
INSERT INTO channel_users (channel_id, user_id) VALUES (2, 1); --Alice/random
INSERT INTO channel_users (channel_id, user_id) VALUES (2, 2); --Bob/general
INSERT INTO channel_users (channel_id, user_id) VALUES (2, 3); --Chris/general
...
INSERT INTO messages (content, user_id, channel_id) VALUES ("Hi, I'm Alice! I forgot what messages needs to be posted.", 1, 1);
...
INSERT INTO messages (content, user_id, channel_id) VALUES ("Hi, I'm Alice! This is messages #10!", 1, 2);

SELECT name from organization;
SELECT name from channel;

SELECT organization.name AS "Organization", channel.name AS "Channel" FROM channel JOIN organization;
SELECT messages.content AS text, post_time AS time FROM messages, channel WHERE messages.channel_id = channel.id AND channel.name = "#general" ORDER BY messages.post_time DESC;
...
SELECT messages.content AS text, user.name AS "User", channel.name AS "Channel" FROM messages, user, channel WHERE messages.user_id = user.id AND user.name = "Alice";
SELECT messages.content AS text, user.name AS "User", channel.name AS "Channel" FROM messages, user, channel WHERE messages.user_id = user.id AND user.name = "Bob" AND channel.name = "#random";
SELECT user.name as "Username", COUNT(messgae.id) AS "Number of messages" FROM messages, user WHERE messages.user_id = user.id GROUP BY user.name;
